...
# 3 manifests
ms = sorted(A.rglob("manifest_*.json")); mrows = []
for m in ms:
...
lines.append(f"5. Tracked-file modifications in working tree: {tracked} (expected only pre-existing ' M .env.example' and the registry/README edit if uncommitted)")
...
rep = "# Phase 1 integrity report (X3-0d)\n\n" + "\n".join(lines) + "\n"
rm.write_new(A / "PHASE1_INTEGRITY_REPORT.md", rep); print(rep)
