...
| **Engineering validation, build B (Paper 1)** | X1-C v2, X1-A v2, X1-B v3 on `sut/X1/build-B` (`beb374f3…`) | Frozen; same agent designed, repaired and re-tested; independent Antigravity methodology review completed after the freeze tag (see section 9) |
...
**Which claims are confirmatory:** none of the human-rater claims. Only the Paper 3 primary and O7 have a design fixed before execution; the X1 protocols were also committed and tagged before their runs, but each v2/v3 rerun was written after the earlier results were seen, and all were self-registered by the executing agent; an independent Antigravity methodology review was completed after the freeze tag (see section 9).

## 2. Experiments and work NOT completed
X2-C confirmatory human round (blocked: no institutional/ethics route, consent, rater provenance); X3-B (not triggered, not started); X1-A FLT-08 (WebSocket) and FLT-09 (audio) (not executed); X1-B2 behavioural stress (decision recorded in `X1_B2_DECISION.md`, not run); Elo/IRT/CAT baseline for Paper 3; verified reading of first-pass literature items labelled SN; edits to `CANONICAL_SCIENTIFIC_TRUTH_CURRENT.md` (proposals only; needs user approval and changelog).
...
## 9. Post-freeze documentation update (2026-09-20)
The freeze tag `freeze/EVIDENCE/FINAL-2026-09-20` and `FINAL_HASH_MANIFEST_FINAL.json` describe the state at the tagged commit. After it, an independent, read-only Antigravity methodology review of X1-A, X1-B-I and X1-C was completed (verdict: METHODOLOGICALLY SOUND ONLY AFTER CLAIM NARROWING; no blocker). It authorizes no rerun and no modification of frozen evidence. Documentation only was then edited to the narrowed wording and to record the review: `PAPER1_FINAL_CLAIM_MATRIX.md`, `manuscript/paper1/*` (abstract facts, contributions, limitations, reproducibility, section outline, result-table notes, figure plan, claim-matrix pointer, title-options note), `FINAL_PUBLICATION_READINESS.md`, `FINAL_STATISTICAL_AUDIT.md`, `FINAL_CLAIM_AUDIT.md`, `X1_OLD_VS_NEW_SUMMARY.md` and this file. No result, protocol, harness, build, tag, run record or manifest was changed, so the manifest hashes for the edited documents no longer match their working-tree bytes; no new freeze has been made. `X1_METHOD_AUDIT.md` (the author's self-audit, tag-covered) and the provisional-freeze documents were not edited and still describe the pre-review state.
