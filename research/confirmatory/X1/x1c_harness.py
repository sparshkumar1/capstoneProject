#!/usr/bin/env python
"""X1-C containment campaign harness (Paper 1). Protocol: research/confirmatory/X1/PROTOCOL_X1-C.md.
...
subprocess shim on the same code path. Everything runs locally against local canaries only.
    ...
    python x1c_harness.py --dry-run                       fixtures only, writes to a temp dir (not evidence)
    python x1c_harness.py --run --registered-commit <40>  official run (protocol tag gate), write-once output
"""
...
PROTOCOL_TAG = "prereg/X1-C/v1"
SUT_TAG = "release/app-repair/v1"
REPS = 5
OUT_REL = "research/confirmatory/X1/results/x1c"
...
def official(argv):
    ...
    for rel in ("research/confirmatory/X1/x1c_harness.py", "research/confirmatory/X1/PROTOCOL_X1-C.md"):
