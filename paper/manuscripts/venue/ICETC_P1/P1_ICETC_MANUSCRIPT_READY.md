...
*Dr. Uma D (Professor), Dept. of CSE, PES University; Naveen S Khadd, Dept. of CSE, PES University; Sparsh Kumar, Dept. of CSE, PES University; Athreya Shashidhara, Dept. of CSE, PES University; Manasa S A, Dept. of CSE, PES University*
...
The pipeline studied here supports technical-interview practice: a learner submits code and written answers, and the system scores them and returns generated feedback. In such formative use, a score is only as trustworthy as the environment that produced it. Submitted code that escapes or damages its sandbox can change the recorded result or the host, model-written feedback that reaches the score fields would let generated text rather than the answer decide the outcome, and an unflagged component failure can be recorded as a genuine zero. These are questions of assessment integrity and of the infrastructure that educational technology relies on. They are not measures of learning: the paper reports no learning, grade, usability or outcome measure.

Existing work supplies methods for each part: sandbox evaluation with fixed programs or adaptive agents judged by host-side signals [1]–[3], [6], [7]; fault injection for services and LLM-agent systems [8]–[10]; and studies of manipulation of LLM graders [11]–[13]. What these studies leave open, for one implemented pipeline, is a property-by-property statement of what was demonstrated, with which oracle, on which system and in which environment, and where the test cannot tell. Each concern here becomes a named property, a test condition, an oracle that does not rely on the component under test and, where possible, a deliberately weakened control that shows the oracle can register a failure.
...
**Environment.** Windows 11 (10.0.26200), WSL2 kernel 6.6.87.2, Docker Desktop with Docker 29.7.2, runc, cgroup v2, 12 CPUs, 8.15 GB memory; the project's local sandbox image; language model `qwen2.5-1.5b-instruct-q4_k_m` with default decoding (temperature 0.1, top-p 0.9) and no fixed seed. No other environment was tested.
...
**Not evaluated.** Real users, learning outcomes, latency guarantees, cross-platform behaviour, adaptive attackers and any property beyond the tested scenarios.

## VIII. Reproducibility and Availability
